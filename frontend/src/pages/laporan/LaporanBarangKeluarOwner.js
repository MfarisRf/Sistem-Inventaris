...
function LaporanBarangKeluarOwner() {
    ...
    const filterData = (data) => {
        ...
        if (month !== '' && year !== '') {
            filtered = data.filter(row => {
                const date = moment(row.tgl_keluar);
        ...
        if (searchTerm !== '') {
            filtered = filtered.filter(row => 
                row.nama_customer.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(`http://localhost:8081/deletelaporankeluar/${id}`);
            Swal.fire({
                icon: "success",
                title: "SUCCESS",
    ...
    const handlePrint = () => {
        ...
        navigate('/cetaklaporankeluar', { state: { filteredData } });
    }
...
export default LaporanBarangKeluarOwner;
