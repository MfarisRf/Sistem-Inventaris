...
export const CetakBarangMasukBulanan = async (req, res) => {
    const { month, year } = req.query;

    // Menambahkan kondisi untuk menyaring data berdasarkan bulan dan tahun
    const sql = `
        SELECT TBM.*, TB.nama_barang, TS.nama_supplier 
        FROM tbl_barang_masuk TBM 
        JOIN tbl_barang TB ON TB.id = TBM.id_barang 
        JOIN tbl_supplier TS ON TS.id = TBM.id_supplier
        WHERE MONTH(TBM.tgl_masuk) = ? AND YEAR(TBM.tgl_masuk) = ?
    `;
